const Main = (function () {
  ...
  function renderSearchResults() {
    ...
    const favSuperHeroes = every.getFavouriteSuperheroes();
  ...
  async function handleSearch(e) {
    ...
    try {
      const data = await every.apiRequest(`${url}/search/${searhTerm}`);
      every.hideLoader();
  ...
  function handleDocumentClick(e) {
    ...
    if (target.classList.contains('add-to-fav')) {
      ...
      every.addHeroToFavourites(hero[0]);
      renderSearchResults();
  ...
  function init() {
    ...
    searchBox.addEventListener('keyup', every.debounce(handleSearch, 500));
